...
class UserRepository implements IUserRepository
{
	...
	private string $baseSelect = 'upper(hex(id)) as id, name, email, cpf, rg, phone, address, zipcode, number, district, city, fu';
	...
	public function delete(Uuid $id): Result
	{
		...
		return ($this->db->affectedRows() > 0)
				...
				: Result::failure("Erro ao excluir o usuário ou usuário não encontrado.");
	...
	public function exists(Email $email, ?Cpf $cpf, ?Uuid $excludeId = null): bool
	{
		...
		if ($excludeId) $builder->where('id !=', "x'{$excludeId}'", false);
