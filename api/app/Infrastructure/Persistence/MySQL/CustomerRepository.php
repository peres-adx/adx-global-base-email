...
class CustomerRepository implements ICustomerRepository
{
  ...
  private string $baseSelect = 'HEX(id) as id, HEX(user_id) as user_id, name, email, cpf, rg, age, phone, address, zipcode, number, district, city, fu';
	...
	public function listByUser(Uuid $userId): Result
	{
		...
		$rows = $this->db->table($this->table)
											->select($this->baseSelect)
											->where('user_id', "UNHEX('{$userId}')", false)
											->get()
											->getResultArray();
  ...
  public function listByCpf(Cpf $cpf): Result
  {
	...
	public function register(Customer $customer): Result
	{
  	...
  	$saved	= $this->db->table($this->table)
												...
												->set('user_id', "UNHEX('{$userId}')", false)
												->set($data)
												->insert();
